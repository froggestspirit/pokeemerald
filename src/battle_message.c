...
// The corresponding WindowTemplate is gStandardBattleWindowTemplates[] within src/battle_bg.c
    ...
    { // 5 Bottom left move (grouped with top left now)
        .fillValue = PIXEL_FILL(0x1),
        .fontId = 8,  // was 7
        .x = 0,
        .y = 0,
    ...
    { // 6 Bottom right move (grouped with top right now)
        .fillValue = PIXEL_FILL(0x1),
        .fontId = 8,  // was 7
        .x = 0,
        .y = 0,
...
void SetPpNumbersPaletteInMoveSelection(void)
{
    ...
    gPlttBufferUnfaded[5] = palPtr[(var * 2) + 0];
    gPlttBufferUnfaded[4] = palPtr[(var * 2) + 1];

    CpuCopy16(&gPlttBufferUnfaded[5], &gPlttBufferFaded[5], sizeof(u16));
    CpuCopy16(&gPlttBufferUnfaded[4], &gPlttBufferFaded[4], sizeof(u16));
}
